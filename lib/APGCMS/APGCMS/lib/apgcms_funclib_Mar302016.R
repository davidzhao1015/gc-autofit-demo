...
###  This will not be used because we move to use the Target/Qualification Ions (Feb 17, 2016)
###
...
compoundIdentify3 <- function(asample.peakInfo, xset.one, lib.peak, alkaneInfo, RI.Variation=0.03, isBLANK=FALSE, print_mzInt=FALSE)
{
  ...
  if (DEBUG) { cat("## After screening RI with Library:", nRI.after,"/", nRI.before," was remained\n");}
...
existInternalStd <- function(internalStd, profiledPeakSet, libDB)
{
   ...
   if( DEBUG ) {
      cat("#\n\nNOTE: If there is a error (argument is of length zero)\n")
