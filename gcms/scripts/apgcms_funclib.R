...
quantificationFunc <- function(f.sample, print.on=FALSE, use.blank, threshold.matchFactor, internalStd, lib.calicurv, cmpdlist, final_PeakProfile_blank, PRINT_MZINT4DB=FALSE, process) 
{
      # RunPlotOnly <- FALSE
      ...
      peaks.sample <- extract_peak_list_samples2(xset.asample, ctype="TIC", offset=OFFSET, plotFile=FALSE)
      ...
      if (DEBUG) {
          ...
          cat("_extractedPeaks_temp.csv file generated.\n")
      }
      ...
      if (DEBUG) {
          ...
          cat("_peak_sample_RI.csv file generated.\n")
      }
      ...
      ##########################################################################    
     {
          if(DEBUG) {
            ...
            write.csv(peak_samples_ri, file=ofilename, quote=FALSE)
            cat("_selectedPeaks_RTRI.csv file generated.\n")
          }
          ...
          if (FALSE && DEBUG) {
            ...
            cat("_PeaksAreaInfo.csv file generated.\n")
          }
    }
    ...
    if (is.null(profiled_peaks)) {
        return(NULL)
    } 
    if(nrow(profiled_peaks) == 0) {
          profiled_peaks <- "\n\nError: There is no matched peak with library. Please check spectrum data file\n"
    ...
    if ( internalStd != 'NONE' & checkInternalStd == TRUE ) {
        ...
        if (process != 'PREPROCESSING'){
            ...
            cat(ofilename, ' generated!', "\n")
        }
        ## concentration summary table
        tmp.Concentration <- finalReport.All[, c("HMDB_ID","Compound","Concentration2","AreaRatio")]
        ...
        colnames(tmp.Concentration) <- c('HMDB_ID', 'Compound', basename(sub(".mzXML|.CDF", "", f.sample, ignore.case = TRUE)), 'AreaRatio' )
...
mergeConcTable <- function( conc.list )
{
      ...
      if (length(conc.each) >= 2) {
          ...
          for (i in 1:length(conc.list)) {
              ...
              # cat ("\n## regexpr: NA\n"); print(regexpr("NA", dt[,3])); print(dt[which(regexpr("NA", dt[,3])>0), ])
              # cat ("\n## regexpr: Multiple peaks\n"); print(regexpr("Multiple peaks", dt[,3])); print(dt[which(regexpr("Multiple peaks", dt[,3])>0), ])
              # cat ("\n## regexpr: <\n"); print(regexpr("<", dt[,3])); print(dt[which(regexpr("<", dt[,3])>0), ])
              
              conc.list[[i]] <- dt[which(dt[,3] == "Internal Standard" | !is.na(as.numeric(dt[,3])) | regexpr("<", dt[,3])>0), ]
              # conc.list[[i]] <- dt
          ...
          # cat("\n\n## conc.list at line 539 at funclib:\n"); print(conc.list);
          ...
          # one sample to clean up result;
          dt <- as.data.frame(conc.list);
          print(dt)
          print(dim(dt))
          if (nrow(dt) != 0){
              conc.all <- dt[which(dt[,3] == "Internal Standard" | !is.na(as.numeric(dt[,3])) | regexpr("<", dt[,3])>0 ), ]
          }else{
              conc.all <- NULL
          }
      }
...
mergeConcTable_new <- function( conc.list )
{
      # excluding NA cases including Multiple Peaks
      for (i in 1:length(conc.list)) {
          dt <- conc.list[[i]]
...
extract_RT_forEachPeak <- function(rt_int_matrix, offset=7, ctype="TIC", xset=NULL) {
      ...
      if (TRUE) {
          ...
          if (DEBUG) {
              sampleFile <- sub(".mzXML|.CDF", "", basename(toString(xset@filepath)), ignore.case=TRUE) 
              ofilename <- paste(sampleFile,"_rt_int_matrix.csv", sep='')
              write.csv(rt_int_matrix, file=ofilename, quote=FALSE)
              cat("_rt_int_matrix.csv file genearted.")
          }
          # xrange <- c(1950:2100)
          # xrange <- c(0:4000)
...
extract_peak_list_blank <- function(xset, ctype="TIC", offset=1.5)  {
    ...
    if( FALSE )  {
        sampleName <- basename(toString(xset@filepath[1]));
...
compoundIdentify4 <- function(asample.peakInfo, xset.one, lib.peak, alkaneInfo, RI.Variation=0.03, isBLANK=FALSE, print_mzInt=FALSE)
{
      ...
      fname.identifeid.trace <- paste(sub(".mzXML|.CDF", "", basename(toString(xset.one@filepath)), ignore.case=TRUE),"_identified.trace.tsv", sep='')  
      flag.firstItem <- TRUE
      
      compounds.list <- list()
      has_identified <- FALSE
      if (nrow(asample.peakInfo) == 0) {
          return(NULL)
      }
      for (j in 1:nrow(asample.peakInfo) ) {
          ...
          if ( RI.sample > 0 ) {  ## not -1  # if -1 then exclude in the previous modules!!!
              ...
              if (length(lib.matched$RI) > 0) {          
                  ...
                  if(DEBUG) { 
                      cat("### candidates count: ",length(lib.matched$RI), "\n");
                  }
                  for(k in 1:length(lib.matched$RI)) {  ## repeat for selected candidates of Library 
                      if(DEBUG) { 
                          cat("### candidates ", k, " SeqIndex: ",lib.matched[k,]$SeqIndex, "\n");
                      }
                      # k<-1
                      ...
                      if( DEBUG & (length(grep("ISTD", lib.matched[k,]$CompoundWithTMS)) > 0) ) {
                          sampleFile <- sub(".mzXML|.CDF", "", basename(toString(xset.one@filepath)), ignore.case=TRUE) 
                      } else {
                          sampleFile <- NULL
                      ...
                      if (DEBUG){
                        cat("####### for HMDB_STD1 and HMDB_STD2","\n");
                        cat("### Match factor score :", MFscore, "\n");
                      }  
                      ...
                      if (DEBUG){
                        cat("RI.similarity: ", RI.similarity, " RIScore: ", RIScore, " matchMZnum: ", matchMZnum, "\n");
                        cat("RI_SIMILARITY_THRESHOLD: ", RI_SIMILARITY_THRESHOLD, " RIScore: ", RIScore, "\n");
                        cat("hmdbID: ", hmdbID, " cor.spearman: ", cor.spearman, "\n");
                        cat("SPEARMAN_CORR_THRESHOLD: ", SPEARMAN_CORR_THRESHOLD, "\n");
                        cat("tmp.TScore: ", tmp.TScore, " TScore: ", TScore, "\n");
                      }
                      ...
                      if ( (RI.similarity > RI_SIMILARITY_THRESHOLD) & (RIScore <= RI.similarity) & (matchMZnum > 10) 
                              & (hmdbID == "HMDB_ISTD1" | hmdbID == "HMDB_ISTD2" | cor.spearman > SPEARMAN_CORR_THRESHOLD) 
              ...
              if (has_identified){
                dir.create("candidates", showWarnings = FALSE)
                candidate_file <- paste("candidates/", 
                                    identified[['hmdbID']], ".tsv", sep='')
                write.table(identified.trace, file=candidate_file, 
                            quote=TRUE, col.names=TRUE,
                            row.names=FALSE, sep="\t", append=FALSE)
                #compounds.list$identified[['hmdbID']] <- identified.trace
                cat("Candidate file ", candidate_file, " generated.", "\n")
                identified.trace <- NULL; ## for trace
                has_identified <- FALSE;
              }
          }
...
calibration <- function(dbLib, hmdbID, relative_area_comp) {
    ...
    if(nrow(libVec) == 1) {
        conc <- calcConcentration(relative_area_comp,
                                  abc=ifelse("a..b..c" %in% colnames(libVec), as.character(libVec$a..b..c), "NA||NA||NA"), 
                                  slopes=libVec$Slope.High..Low. , 
                                  intercepts=libVec$Intercept.High..Low. , 
                                  concentration_threshold=libVec$Concentration_threshold.uM., 
                                  loq.value=libVec$LOQ) # loq.value: uM
    } else {
        if(nrow(libVec) > 1) {
...
# return: uM conc
calcConcentration <- function(area, abc, slopes, intercepts, concentration_threshold, loq.value=NULL)
{  
  # if ( is.na(abc) ) {
      #if (DEBUG) {
        cat("# [calcConcentration]  abc:", as.character(abc), "\n")
      #}
  #}

  if( is.na(slopes) | is.na(intercepts) ) {
      if(DEBUG) {
          cat("# [calcConcentration] area:", area,"\t slope:", slopes, "\t intercept:", intercepts,"\n")
          cat("\t Slope & Intercept are NA\n")
      }
  }
  if ( is.na(slopes) | is.na(intercepts) | is.na(abc) ) {
      return ("NA (No Calibration Curve)");
  }  
  abc_v <- unlist(strsplit(as.character(abc), "[|][|]")) 
  slopes_v <- unlist(strsplit(as.character(slopes), "[|][|]"))
  intercepts_v <- unlist(strsplit(as.character(intercepts), "[|][|]"))
  
  if (!grepl("NA", abc_v[1], ignore.case=TRUE) &&
      !grepl("NA", abc_v[2], ignore.case=TRUE) &&
      !grepl("NA", abc_v[3], ignore.case=TRUE) && 
      grepl("NA", slopes_v[1], ignore.case=TRUE)) {
     return ("NA (No High Calibration Curve)")
  }

  slope <- as.numeric(slopes_v[1])
  intercept <- as.numeric(intercepts_v[1])
  loq.calc <- NULL
  conc <- NULL
  if ( area > 0 ) {
      if ( !grepl("NA", abc_v[1], ignore.case=TRUE) && 
           !grepl("NA", abc_v[2], ignore.case=TRUE) && 
           !grepl("NA", abc_v[3], ignore.case=TRUE) ){
         square <- 4 * as.numeric(abc_v[1]) * area + (as.numeric(abc_v[2]))^2 - 4 * as.numeric(abc_v[1]) * as.numeric(abc_v[3])
         if ( square >= 0 ) {
             conc <- round((square^0.5 - as.numeric(abc_v[2])) / (2 * as.numeric(abc_v[1])), 2)
         }
         else {
             conc <- NULL
         }
      }
      if ( is.na(conc) || is.null(conc) || conc < as.numeric(concentration_threshold) ) {   
         conc <- round( (area - intercept) * 1000 / slope , 2); # unit: uM
         if (! grepl("NA", concentration_threshold, ignore.case=TRUE) && 
               conc < as.numeric(concentration_threshold)  &&   
            ! grepl("NA", slopes_v[2], ignore.case=TRUE) && 
            ! grepl("NA", intercepts_v[2], ignore.case=TRUE) ) {
            slope2 <- as.numeric(slopes_v[2])
            intercept2 <- as.numeric(intercepts_v[2])
            conc <- round( (area - intercept2) * 1000 / slope2 , 2); # unit: uM
         }
      }
  } else {
          conc <- "Area is negative."
  }
...
quantification <- function(profiled.result, lib, lib.curve, internalStdCmpd, sample_file, stype) {
    ...
    if (length(area.internalStd) == 0 ) {
        stopMessage(paste("Error: Cannot find internal standard compound '", 
              internalStdCmpd, 
              "' in your sample. Make sure your sample has that compound.") )
    }
...
existInternalStd <- function(internalStd, profiledPeakSet, libDB)
{
   ...
   if( ! (internalStd %in% profiledPeakSet$CompoundWithTMS) ) {
      ...
      amsg <- paste("Error: Cannot find the selected internal standard (",internalStd,") in the Sample Spectrum.\n")
   ...
   if (FALSE)  {
       tryCatch({
           ...
           if( ! (internalStd %in% as.character(libDB$CompoundWithTMS)) ) {
             ...
             # showErrMessage("\n\n## Warning: Can not find the internal standard (",internalStd,") in the library database.\n")
             amsg <-paste("\n\n## Warning: Cannot find the selected internal standard (",internalStd,") in the library database.\n")
             cat(amsg)
             cat(file=File.ErrorLog, amsg, append=TRUE)
             # stop("\n\n## Warning: Can not find the internal standard (",internalStd,") in the library database.\n\t Program will report without quantification.\n\n")     
             # return (FALSE)
